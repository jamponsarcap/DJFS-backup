...
import { Upload, FileText, X, Database, Search, CheckCircle, XCircle, Clock } from 'lucide-react'
import { uploadStatement, fetchUploadHistory } from '../api/client'
...
export default function DocumentUpload({ clientId, onUploadComplete, historyRefreshKey }: Props) {
  ...
  useEffect(() => {
    fetchUploadHistory(clientId)
      .then(h => { setUploadHistory(h); setShowHistory(h.length > 0) })
      .catch(() => setUploadHistory([]))
  }, [clientId, historyRefreshKey])
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-xl p-6 text-center transition-colors
          ${processing ? 'cursor-not-allowed opacity-60 border-gray-200' :
      ...
      {/* Upload history */}
      {uploadHistory.length > 0 && !processing && showHistory && (
